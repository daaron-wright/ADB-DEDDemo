...
interface SimpleQuestionnaireProps {
  ...
  onItemUpdate?: (
    sectionId: string,
    itemId: string,
    status: QuestionnaireItem["status"],
  ) => void;
}
...
const RadioIcon: React.FC<{ selected?: boolean }> = ({ selected = false }) => (
  ...
  >
    <path
      ...
      fill="white"
    />
    {selected && <circle cx="16" cy="16" r="6" fill="#54FFD4" />}
  </svg>
);
...
const SimpleQuestionnaire = forwardRef<
  SimpleQuestionnaireHandle,
  SimpleQuestionnaireProps
>(({ journeyNumber, completedCount, totalCount, onItemUpdate }, ref) => {
  ...
  const handleItemClick = (
    sectionId: string,
    itemId: string,
    currentStatus: QuestionnaireItem["status"],
  ...
  const updateItemStatus = (
    sectionId: string,
    itemId: string,
    newStatus: QuestionnaireItem["status"],
  ) => {
    setSections((prev) =>
      prev.map((section) =>
        ...
        <div className="space-y-0">
          {sections.map((section, index) => (
            <div key={section.id} className="relative">
              <div
                className={`px-8 ${index === 0 ? "pt-6 pb-8" : index === 1 ? "py-8" : "pt-8 pb-8"}`}
              >
                {/* Section Title */}
                    ...
                    <motion.div
                      ...
                      className={cn(
                        ...
                        highlightedItem === item.id
                          ? "bg-white/10 ring-2 ring-[#54FFD4]"
                          : "hover:bg-white/6",
                      )}
                      ref={(element) => {
