...
function buildFinalDecisionRejectionNarrative(
  ...
  const arabicLines = [
    ...
    "7. وكيل اقتراح الا��م → إرشاد. جهّز المبررات الداعمة قبل التصعيد.",
  ];
...
function summarizeSimilarityRawDetail(
  ...
  if (results.length > 0) {
    ...
    if (isPlainObject(primaryResult)) {
      ...
      if (resultReason) {
        sentences.push(`Matching rationale: ${wrapInSmartQuotes(resultReason)}.`);
      }
...
>(function VerificationStepItem(
  ...
  const renderAgentNarrative = (
    ...
    const accordionDefaultValue = hasNarrative ? ["agent-responses"] : undefined;
