...
type AgentRawDetail = string | Record<string, unknown> | undefined;
...
type TradeNameVerificationStep = {
  ...
  rawDetailSuccess?: AgentRawDetail;
  rawDetailFailure?: AgentRawDetail;
};
...
const TRADE_NAME_CHECKS: ReadonlyArray<TradeNameVerificationStep> = [
    ...
    rawDetailSuccess: {
      ...
      data: {
        ...
        validation_summary: {
          ...
          ilm_judge_enabled: true,
          processing_time_ms: 646.7037200927734,
          suggested_next_agent: "Prohibited words agent",
        },
        text: "Marwa Restaurant",
        language: "english",
        normalized: "Marwa Restaurant",
      },
    },
    rawDetailFailure: {
      success: true,
  ...
  },
  {
    ...
    rawDetailSuccess: {
      success: true,
      message: "Restricted-term scan completed.",
      data: {
        is_valid: true,
        total_violations: 0,
        violations: [],
        validation_summary: {
          ...
          semantic_matching_enabled: true,
          ilm_judge_enabled: true,
          processing_time_ms: 512.431640625,
          suggested_next_agent: "Similarity agent",
        },
        text: "Marwa Restaurant",
        language: "english",
        normalized: "Marwa Restaurant",
  ...
  },
  {
    ...
    failureDetail: {
      ...
      ar: [
        ...
        '7. وكيل اقتراح الاسم (الاس�� المرفوض) → إرشاد. الخيار الموصى ��ه: "ساحة الختيار".',
      ].join("\n"),
    },
    rawDetailSuccess: {
      success: true,
  ...
  },
  {
    ...
    rawDetailSuccess: {
      success: true,
      message: "Buckwalter validation completed.",
      data: {
        arabic_name: "مطعم مروة",
        english_name: "Marwa Restaurant",
        transliterated_english: "mTEm mrw",
        validation_passed: true,
        confidence_score: 0.95,
        method: "CAMeL Tools + FuzzyMatcher + LLM Semantic Fallback",
        details: "Validation PASSED (phonetic); MODE B DETECTED.",
      },
    },
    rawDetailFailure: {
      english: "Bait El Khetyar",
      arabic: "بيت الختيار",
      buckwalter_score: 0.95,
      status: "on_hold",
      pending_reason: "Resolve similarity conflict first",
    },
  },
  {
    ...
    rawDetailSuccess: {
      success: true,
      message: "Activity validation completed.",
      data: {
        trade_name: "Marwa Restaurant",
        activity_alignment: "Food & Beverage Restaurant",
        inference_mode: "Mode A",
  ...
  },
  {
    ...
    successDetail: {
      ...
      ar: [
        ...
        "• وكيل الكلمات المحظو��ة → ��اجح. لم يتم العثور على مصطلحات م��ظورة في النس����تين العربية والإنجلي��ية.",
    ...
    rawDetailFailure: {
      ...
      summary: "Conflicts with registered Bait Al Khetyar",
    },
  },
  {
    ...
    rawDetail: {
      trade_name: "Bait El Khetyar",
      strategy: "Add distinguishing culinary descriptors",
      candidates: [
        "Bait El Khetyar Heritage Kitchen",
        "Khetyar Dining House",
        "House of Khetyar",
      ],
    },
  },
...
const RESTAURANT_ACTIVITY_ID = "fnb-restaurant";
const HERITAGE_ACTIVITY_ID = "heritage-retail";

const ACTIVITY_COMPATIBILITY_OPTIONS: ActivityOption[] = [
  { id: RESTAURANT_ACTIVITY_ID, label: "Food & Beverage Restaurant" },
  { id: HERITAGE_ACTIVITY_ID, label: "Heritage Retail Concept" },
...
const STAGE_PROGRESS_EPSILON = 0.0001;
...
function buildFinalDecisionRejectionNarrative(
  ...
  const arabicLines = [
    ...
    "6. محرك القرار النهائي → تم التصعيد للمراجعة. لسنا واثقين من ا��رفض الآلي، لذلك تم رفعه لمراجع ��ائرة التنمية ال��قتصادية لتحديد الإجراء.",
...
function generateAlYahGuidanceFromFailure(
  detail?: string | LocalizedAgentNarrative,
): string {
...
function summarizeActivityRawDetail(detail: Record<string, unknown>): string[] {
  ...
  if (llmJudgment) {
    ...
    if (llmDecision) {
      const confidenceDisplay = formatPercentage(llmConfidence);
      sentences.push(
        `Al Yah issued an ${llmDecision.toLowerCase()} decision${
          confidenceDisplay ? ` with ${confidenceDisplay} confidence` : ""
        } after manual review.`,
      );
    }
...
>(function VerificationStepItem(
  ...
  const hasStepRawDetail =
    typeof step.rawDetail === "string"
      ? step.rawDetail.trim().length > 0
      : Boolean(step.rawDetail);
  const hasFailureNarrative = Boolean(step.failureDetail) || hasStepRawDetail;
  const hasSuccessNarrative = Boolean(step.successDetail) || hasStepRawDetail;

  const failureNarrativeKey = React.useMemo(() => {
  ...
  const renderAgentNarrative = (
    ...
    const rawDetailSummary = summarizeAgentRawDetail(
      rawDetail,
      displayOptions?.focusTitle ?? null,
    );
    const hasRawSummary = rawDetailSummary.length > 0;
    ...
    if (!hasNarrative && !hasRawSummary) {
      return null;
    }

    if (shouldHideStructuredResponses && !hasRawSummary) {
      return null;
    }
    ...
    const accordionDefaultValue = undefined;
  ...
  const alYahPrompt = React.useMemo(() => {
    const guidance = generateAlYahGuidanceFromFailure(step.failureDetail);
                ...
                {renderAgentNarrative(
                  ...
                  step.title === "Full decision flow"
                    ...
                    : {
                        ...
                        rawDetail: step.rawDetail,
                        hideStructuredResponses:
                          (step.title === "Similarity agent" &&
                            !showActivitySelectorsInline) ||
...
export function BusinessRegistrationFocusContent({
  ...
  const updateThinkingToast = React.useCallback(
    (stageIndex: number, phase: "start" | "complete" | "failure" = "start") => {
      ...
      const description = messages
        ...
        : "Al Yah is reviewing the trade name.";
  ...
  React.useEffect(() => {
    ...
    const stageIndex = Math.min(
      Math.floor(
        ((automationProgress + STAGE_PROGRESS_EPSILON) / 100) * totalStages,
      ),
      totalStages - 1,
    );
    const tracker = stageProgressRef.current;
  ...
  const finalDecisionSuccessDetail =
    finalDecisionStepDefinition?.successDetail ?? null;
  const finalDecisionSuccessRawDetail =
    finalDecisionStepDefinition?.rawDetailSuccess ?? null;
  const finalDecisionFailureRawDetail =
    finalDecisionStepDefinition?.rawDetailFailure ?? null;
      ...
      description:
        decisionStatus === "completed"
          ...
          : decisionStatus === "failed"
            ...
            : "Al Yah is walking through each check and narrating what happens.",
      summary: "Plain-English view of how Al Yah is progressing.",
      status: decisionStatus,
                      ...
                      {automationSteps.map((step, index) => {
                        ...
                        return (
                          <VerificationStepItem
                            ...
                            shouldAutoOpenNarrative={
                              shouldShowActivityOptions && !isActivityStep
                            }
                          />
                        );
                      })}
