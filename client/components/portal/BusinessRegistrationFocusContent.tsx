  ...
  },
  {
    ...
    failureDetail: {
      ...
      ar: [
        ...
        "2. وكيل الكل��ات ��لمحظورة → ناجح. لم يتم رصد مفردات محظور�� في النسختين ��لعربية أو الإنجليزية.",
  ...
  },
  {
    ...
    successDetail: {
      ...
      ar: [
        ...
        '• مدقق النص / التدقيق الإملائي / الف��ص الثقافي → ناجح. تم توحيد "Marwa Restaurant" والتأكد من الملاءمة ����لثقافية.',
...
function buildSimilarityConflictNarrative(
  ...
  const arabicLines = [
    ...
    `6. محرك القرا�� النهائي → مرفوض. مرج�� التعا��ض ${SIMILARITY_CONFLICT_REFERENCE}؛ يُرجى اقتر���ح اسم مختلف.`,
    hasIteration
...
function buildFinalDecisionRejectionNarrative(
  ...
  const arabicLines = [
    ...
    "6. محرك القرار النهائي → ��م التصعيد للمراجعة. لست واثقًا من الرفض الآل�� لذلك تم ��فعه لمراجع دائرة التنمية الاقت��ادية للتوجيه.",
      ...
      <div className="space-y-3 rounded-2xl border border-dashed border-[#e6f2ed] bg-white px-4 py-3 text-sm text-slate-600">
        ...
        {isFailed && step.failureDetail ? (
          <div className="space-y-3">
            ...
            {isFailed && onEscalate && !showFinalDecisionEscalationControl ? (
              <button
                type="button"
  ...
  const beginStageRun = React.useCallback(() => {
    ...
    setStageStatuses(
      TRADE_NAME_CHECKS.map((_, index) =>
        (index === 0 ? "current" : "pending") as TradeNameCheckStatus,
      ),
    );
    ...
    if (stageIndex > tracker.currentIndex) {
      ...
      setStageStatuses((previous) =>
        previous.map((status, index) => {
          ...
          return (status === "failed" ? "failed" : "pending") as TradeNameCheckStatus;
        }),
      );
