...
export function ReviewFocusSheet({ open, review, onClose }: ReviewFocusSheetProps) {
  if (!review) {
    return null;
  }

  const timeline = [
    { label: 'Submitted', value: dateFormatter.format(new Date(review.submittedAt)) },
    ...
    <Sheet open={open} onOpenChange={(value) => !value && onClose()}>
      ...
      >
        <SheetHeader className="space-y-3 px-6 pb-4 pt-6 text-left">
          <Badge className={`inline-flex w-fit items-center gap-2 border px-3 py-1.5 text-xs font-semibold uppercase tracking-[0.18em] ${priorityStyles[review.priority]}`}>
          ...
          <div className="flex flex-wrap items-center gap-3 text-slate-900">
            ...
            <Badge className={`border px-3 py-1.5 text-xs font-semibold uppercase tracking-wide ${slaStyles[review.slaStatus]}`}>
              {review.slaStatus}
            </Badge>
        ...
        <ScrollArea className="flex-1">
          <div className="space-y-8 px-6 py-6">
            <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {dossier.map((item) => (
                <div key={item.label} className="rounded-2xl border border-[#d8e4df] bg-[#f9fbfa] px-4 py-3">
            ...
            <section className="grid gap-4 md:grid-cols-3">
              {timeline.map((item) => (
                <div key={item.label} className="rounded-2xl border border-[#d8e4df] bg-white px-4 py-3">
                  <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#0f766e]">
                    {item.label}
                  </p>
                  <p className="mt-2 text-sm font-semibold text-slate-900">{item.value}</p>
              ...
              <div className="rounded-2xl border border-[#d8e4df] bg-white px-4 py-3">
                <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#0f766e]">Completion</p>
                <div className="mt-3 flex items-center gap-3">
                  <Progress value={review.completion} className="h-2 flex-1 overflow-hidden rounded-full bg-[#e3efea]" />
                  <span className="text-sm font-semibold text-[#0f766e]">{review.completion}%</span>
                </div>
              </div>
            </section>

            <section className="space-y-3">
              <h3 className="text-sm font-semibold uppercase tracking-[0.18em] text-[#0f766e]">Reviewer notes</h3>
            ...
            <section className="space-y-3">
              <h3 className="text-sm font-semibold uppercase tracking-[0.18em] text-[#0f766e]">Suggested follow-up</h3>
              <div className="rounded-2xl border border-[#d8e4df] bg-white px-5 py-4 text-sm text-slate-700">
                <ul className="list-inside list-disc space-y-2">
                  <li>Confirm outstanding documents and notify {review.assignedTo} of updated SLA expectations.</li>
                  <li>Review supporting files ({review.attachments}) for compliance deviations before next stage.</li>
                  <li>Coordinate with the {review.directorate} team for any dependencies impacting the business journey.</li>
                </ul>
              </div>
            </section>
          </div>
