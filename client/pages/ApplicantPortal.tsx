...
const DEFAULT_WORKSPACE_TITLE = PRIMARY_TRADE_NAME_EN;
...
const BUSINESS_AI_INTRO_MESSAGES: Record<PortalLanguage, string> = {
  ...
  ar: "قبل أن أعرض تفاصيل الترخيص، ��عي��ا نؤكد معًا الشكل القانوني الأنسب، وأ��شطة العمل، ومتطل���ات المسا��ة. بمجرد إ��هاء ه��ا الاستبيان، سأعرض نوع الترخيص ومعرّف الطلب.",
};
...
interface PortalLanguageCopy {
  ...
  tradeNamePaymentNextStep: string;
  tradeNamePaymentToast: string;
...
type ApplicationNextActionState = "default" | "pay-trade-name";
...
const PORTAL_LANGUAGE_COPY: Record<PortalLanguage, PortalLanguageCopy> = {
  ...
  ar: {
    ...
    heroTitle: "رحلتك مدعومة بالذكاء ا��اصطنا��ي",
    heroDescription: (name: string) =>
    ...
    applicationSummaryNote:
      "سيق��م مساعد الذكاء الاصطناعي ت��قائيًا بجلب عقد الإيجار من نظام بلدية أبوظبي فو�� تسجيل ��قدك.",
    businessAITitle: "بولاريس",
    businessActivityGuidance:
      "يمكنك اختيار عدة أ��شطة تجارية للمطعم، بشرط أن تنتمي إلى نف�� مجموعة الأعما��. يمكنك إدراج ما يصل إلى 10 أنشطة في رخصة تجارية واحدة.",
    businessActivityGuidanceLabel:
...
const STAGE_PHASE_MAP: Record<string, JourneyPhaseId> = {
  [QUESTIONNAIRE_STAGE_ID]: "ideate",
  "trade-name-activities": "apply",
...
export default function ApplicantPortal() {
  ...
  const phaseToastRef = useRef<JourneyPhaseId | null>(null);
  ...
  }, [activeStageId, primaryApplication.id, toast]);
  ...
  const handlePolicyUpdateAcknowledged = useCallback(() => {
...
const journeyTimelineItems = useMemo<JourneyTimelineItem[]>(() => {
  ...
  const journeyFocusViewProps = useMemo(() => {
    ...
    const focusViewProps: JourneyStageFocusViewProps = {
      ...
      onTradeNameSelected: handleTradeNameSelectedNotification,
      onTradeNameReservationSubmitted: handleTradeNameReservationSubmitted,
      tradeNamePayAndIssueLabel: languageCopy.tradeNamePaymentNextStep,
      tradeNamePayAndIssueToast: languageCopy.tradeNamePaymentToast,
      growthUnlocked:
        stageId === "compliance-growth" ? priorStagesComplete : undefined,
    };

    if (stageId === "questionnaire") {
      focusViewProps.legalFormOptions = DEFAULT_LEGAL_FORM_OPTIONS;
      focusViewProps.selectedLegalFormId = selectedLegalFormId;
      focusViewProps.onLegalFormSelect = setSelectedLegalFormId;
    }
  ...
  const heroSection = (
    <section
      className={cn(
        "relative overflow-hidden rounded-3xl bg-gradient-to-br from-[#0d3f43] via-[#0f5f66] to-[#0f766e] p-6 text-white shadow-[0_24px_48px_-32px_rgba(11,64,55,0.35)] sm:p-8",
        focusClasses.hero,
      )}
    >
      <div
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          <Button
            ...
            className={cn(
              ...
              focusClasses.questionnairePrimaryCta,
            )}
          >
